...
table(is.na(cccu_complete1$contra_satis))#still high missingness
...
table(cccu_complete1$exclusion_missing_selection_healthprb)#missings
...
table(cccu_complete1$exclusion_missing_selection_feelpg)#missings
...
table(cccu_complete1$exclusion_missing_selection_avoid)#missings
...
table(cccu_complete1$NOBARRIER)#no non-hormonal contraceptive method in last 30days
table(cccu_complete1$hc)#no hormonal contracepive method 
#joint = no contraceptive method 
...
crosstabs(~exclusion_R2 + hc + NOBARRIER + sex_inlast30days, data = cccu_complete1) #seems to have worked :) :) (see NOBARRIER = (1) Yes, sex_inlast30days = Yes)
...
cccu_complete1 <- cccu_complete1 %>%
  mutate(exclusion_R3 = ifelse(VASECTOMY == "(1) Yes" |
                                 SP1VAS == "(1) Yes" | SP2VAS == "(1) Yes", 1, ifelse (VASECTOMY == "(0) No" |
                                 SP1VAS == "(0) No" | SP2VAS == "(0) No", 0, NA)))
