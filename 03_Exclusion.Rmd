...
Fake_cccu_complete1$contra_satis = ifelse(is.na(Fake_cccu_complete1$contra_satis),1,Fake_cccu_complete1$contra_satis)
...
table(Fake_cccu_complete1$exclusion_missing_selection_healthprb)#missings
...
table(Fake_cccu_complete1$exclusion_missing_selection_preg_bet_waves)#a lot of missings -> something wrong?
...
table(Fake_cccu_complete1$exclusion_missing_selection_baby_bet_waves)#a lot of missings -> something wrong?
...
table(Fake_cccu_complete1$exclusion_missing_selection_feelpg)#missings

#Avoidance of pregnancy
Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
 mutate(exclusion_missing_selection_avoid= as.factor(ifelse(is.na(Avoid_recode),1,0))) 

table(is.na(Fake_cccu_complete1$Avoid_recode))
table(Fake_cccu_complete1$exclusion_missing_selection_avoid)#lot missings -> something wrong?
...
Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
 mutate(exclusion_IUD_next = ifelse(hc_next == "IUD&non_hc", 1,0))
...
Fake_cccu_complete1$hc_dur = ifelse(is.na(Fake_cccu_complete1$hc_dur),1,Fake_cccu_complete1$hc_dur)
...
table(Fake_cccu_complete1$hc)#no hormonal contraceptive method 
...
qplot(Fake_cccu_complete1$exclusion_R2)#missingness might be because sex_inlast30days and NOBARRIER can be missing 

crosstabs(~exclusion_R2 + hc + NOBARRIER + sex_inlast30days, data = Fake_cccu_complete1) #seems to have worked :) :) (see NOBARRIER = (1) Yes, sex_inlast30days = Yes)

```
-> should keep an eye on missingess in exclusion columns (if we exclude based on filter(exclusion_missing_XXX !=1) or filter(exclusion_missing_XXX == 0 | is.na(XXX))
