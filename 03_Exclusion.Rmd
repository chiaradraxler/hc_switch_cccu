...
@Laura@ -> die Kombi IUD und non_hormonal scheint nach unserer Codierung nicht zu existieren? 
```{r}
table(Fake_cccu_complete1$hc)
...
#IUD & non-hormonal 
Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
 mutate(exclusion_IUD = ifelse(IUD == "(1) Yes" & hc == "non_hc", 1,0)) 
...
table(Fake_cccu_complete1$IUD_next)
Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
 mutate(exclusion_IUD_next = ifelse(IUD_next == "(1) Yes" & hc == "non_hc", 1,0))
...
x = Fake_cccu_complete1 %>%
  select(IUD_next, hc, exclusion_IUD_next)
```
