...
length(unique(Fake_cccu_complete1$CASEID)) -
  Fake_cccu_complete1 %>%
  filter(exclusion_used_EC == 0) %>%
  select(CASEID) %>%
  unique() %>%
  nrow() 


table(is.na(Fake_cccu_complete1$exclusion_used_EC))
...
```
working from now with Fake_cccu_complete2
##3.2 No missingness
##3.2.1 NO Missingness IUD and IUDnext
```{r}

#column that indicates missingness in IUD
Fake_cccu_complete2 <- Fake_cccu_complete2 %>%
...
Fake_cccu_main_analysis <-Fake_cccu_complete2 %>%
...
table(is.na(Fake_cccu_main_analysis1$switch))
...
Fake_cccu_main_analysis1 <- Fake_cccu_main_analysis1 %>%
 mutate(exclusion_missing_predHC = ifelse(is.na(hc), 1, 0),
        exclusion_missing_selection = ifelse(is.na(AGE) | is.na(DEGREE_recode), 1, 0))
   
   
   # sum = sum(is.na(Fake_cccu_main_analysis1$switch)+
   ...
   #                                         is.na(Fake_cccu_main_analysis1$rel_dur_factor)),
   # exclusion_missingness = ifelse(sum
...
table(is.na(Fake_cccu_main_analysis1$exclusion_missingness))
...
```{r}
#overview
Fake_cccu_MA = Fake_cccu_complete1
table(Fake_cccu_MA$VASECTOMY)
table(Fake_cccu_MA$SP1VAS)
table(Fake_cccu_MA$SP2VAS)
crosstabs(~exclusion_R3+SP2VAS, data = Fake_cccu_MA)
...
x = Fake_cccu_MA %>% filter(SP2VAS == "(1) Yes") %>% select(VASECTOMY, SP1VAS, SP2VAS, exclusion_R3)
