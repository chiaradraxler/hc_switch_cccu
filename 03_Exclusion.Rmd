...
table(Fake_cccu_complete1$exclusion_missing_pred_contra_dura)
Fake_cccu_complete1$exclusion_missing_pred_contra_dura- as.factor(Fake_cccu_complete1$exclusion_missing_pred_contra_dura)
qplot(Fake_cccu_complete1$exclusion_missing_pred_contra_dura)
...
length(unique(Fake_cccu_complete1$CASEID)) -
  Fake_cccu_complete1 %>%
  filter(exclusion_missing_pred_sex_freqs !=1) %>%
  select(CASEID) %>%
  unique() %>%
  nrow()
...
Fake_cccu_SA <-Fake_cccu_SA1 %>%
  filter(exclusion_missing_pred_contra_dura !=1)

#number of excluded women from Fake_cccu_complete1 bc of exclusion_missing_pred_contra_dura = 1 (might be combination with other exclusion criteria)
