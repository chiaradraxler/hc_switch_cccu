...
# 5. Senstivity analysis after m4: controlling for unobserved selection effects
```{r}
#repeat models first
...
exp(m3$coefficients)
exp(confint(m3, method = "Wald", level = .997))

#model 4
exp(m4$coefficients)
exp(confint(m4, parm = "contra_satis", method = "Wald", level = .997))
exp(confint(m4, parm = "hcnon_hc:contra_satis", method = "Wald", level = .997))

exp(confint(m4, parm = "hcnon_hc", method = "Wald", level = .997))
#compute e-values

#model 3

#for contra_satis
#evalue(OR(,rare=FALSE),lo= ,  hi=)
