...
```{r}
m1 <- glmer(switch ~ hc + (1| CASEID), family = binomial, data = Fake_cccu_MA)
...
```{r}
m2 <- glmer(switch ~ hc*contra_satis + (1| CASEID), family = binomial, data = Fake_cccu_MA)
...
```{r}
m3 <- glmer(switch ~ hc * contra_satis + hc * sexual_satisfaction + hc * sex_freq + (1| CASEID), family = binomial, data = Fake_cccu_MA)
