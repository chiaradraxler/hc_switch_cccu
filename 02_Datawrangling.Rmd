...
-> we can use variable hc (contra_satis should follow it) because this variable classifies into hc or non hc and then values should be compared -> it does not matter if one is 1x or 2x hc user or 1x hc and 1x non hc and 2x non hc because hc classifies 
hc = Wenn ich - 1x hormonell + 1x nicht-hormonell vorliegen habe (betrifft auch 1 mal nur hormonell) dann nehme ich die Zufriedenheit mit der spezifischen hormonellen Methode 

Wenn ich 2x hormonell vorliegen habe, dann nehme ich den höherer Zufriedenheitswert bei den jeweiligen beiden hormonellen Methoden

non_hc = Wenn ich 1x oder 2x nicht-hormonell vorliegen habe, dann nehme ich den höherer Zufriedenheitswert bei den jeweiligen beiden nicht-hormonellen Methoden.

and we should then have satisfaction values based on all used variables. 
if there is only 1 used pmax copies it, if there are 2 pmax compares 
...
Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
  mutate(contra_satisfaction = ifelse(hc == "hc",
                                      pmax(Q23D_PILL, Q23D_PATCH, Q23D_DEPO,
                                           Q23D_IMPLANT, Q23D_RING,
                                           na.rm = TRUE),
                                      pmax(RSAT1, RSAT2, RSAT3, RSAT4, RSAT5,
                                           na.rm = TRUE)))

Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
  mutate(contra_satisfaction = contra_satisfaction - 1)
table(Fake_cccu_complete1$contra_satisfaction)

x = Fake_cccu_complete1 %>%
  select(hc, Q23D_PILL, Q23D_PATCH, Q23D_DEPO,
         Q23D_IMPLANT, Q23D_RING,RSAT1, RSAT2, RSAT3, RSAT4, RSAT5, contra_satisfaction)
```
