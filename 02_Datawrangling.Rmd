...
x = Fake_cccu_complete1 %>%
  select(hc, Q23D_PILL_r, Q23D_PATCH_r, Q23D_DEPO_r,
         Q23D_IMPLANT_r, Q23D_RING_r,RSAT1_r, RSAT2_r, RSAT3_r, RSAT4_r, RSAT5_r, contra_satisfaction)
```
...
-> we have t1: years & month duration
            ...
            --> then add t1 + t2 usw...
```{r}
#higher duration value:  years
Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
  mutate(hc_duration_y = pmax(YPILL, YPATCH, YRING,
        YDEPO, YIMPLANT, na.rm = TRUE)) 

#higher duration value: months
Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
  mutate(hc_duration_m = pmax(MPILL, MPATCH, MRING,
        MDEPO, MIMPLANT,  na.rm = TRUE)) 

#now combine values (months)

Fake_cccu_complete1 <- Fake_cccu_complete1 %>%
  mutate(hc_dur = hc_duration_y + hc_duration_m)

#now add values t1 + t2-4
...
xx = Fake_cccu_complete1 %>%
  select(hc, YPILL, YPATCH, YRING,
        YDEPO, YIMPLANT,MPILL, MPATCH, MRING,
        MDEPO, MIMPLANT, hc_dur,hc_duration_y, hc_duration_m )
