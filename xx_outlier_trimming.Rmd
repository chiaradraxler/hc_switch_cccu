...
# Continous variables
```{r}
mean_contra_satis = mean(cccu_MA$contra_satis, na.rm = T)
sd_contra_satis = sd(cccu_MA$contra_satis, na.rm = T)

cccu_MA_trim = cccu_MA %>%
  mutate(contra_satis_trim = ifelse(contra_satis > mean_contra_satis + 3*sd_contra_satis |
                                      contra_satis < mean_contra_satis - 3*sd_contra_satis,
                                    mean_contra_satis,
                                    contra_satis))

boxplot(cccu_MA_trim$contra_satis_trim)$out
```


```{r}
mean_POVRATE = mean(cccu_MA$POVRATE, na.rm = T)
sd_POVRATE = sd(cccu_MA$POVRATE, na.rm = T)

cccu_MA_trim = cccu_MA %>%
  mutate(POVRATE_trim = ifelse(POVRATE >= mean_POVRATE + 3*sd_POVRATE |
                                      POVRATE <= mean_POVRATE - 3*sd_POVRATE,
                                    mean_POVRATE,
                                    POVRATE))

boxplot(cccu_MA_trim$POVRATE_trim)$out
```
...
m4 <- glmer(switch ~ hc * contra_satis_trim + hc * sexual_satisfaction_trim +  hc * sex_freq + hc *AGE_trim + hc *DEGREE_recode + hc *POVRATE_trim + hc *HLTHPROB_recode + hc *MEDPROB_recode +  hc *GAPINS_recode +
            ...
            family = binomial,
            data = cccu_MA, control=glmerControl(optimizer="bobyqa"))
```
